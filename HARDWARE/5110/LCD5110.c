...
void LcdPortInit(void)
{
  ...
  GPIO_InitStructure.GPIO_Pin = LCD_SCLK_BIT | LCD_SDIN_BIT | LCD_DC_BIT | LCD_SCE_BIT | LCD_REST_BIT;
...
void PutWideChar(u8 value,u8 column,u8 row)
{
	u8 i;
	//写上半行
	LCD_set_XY(row,column << 1);
	for(i = 0;i < 8;i ++)
		LcdWrite(ASCII_8_16[value - 0x20][i],DATA);
	//写下半行
	LCD_set_XY(row,(column << 1) + 1);
	for(i = 8;i < 16;i ++)
		LcdWrite(ASCII_8_16[value - 0x20][i],DATA);
}
...
void LCD_Write_Num(unsigned char X,unsigned char Y,u32 num,unsigned char N)
{
  unsigned char line;
  unsigned char i=0;
  u8 n[5]={0};
  ...
  for(i=N;i>0;i--) 
  {
    ...
    for (line=0; line<6; line++)
      LcdWrite(ASCII_6_8[n[i-1]][line], DATA);//从ACSII码表中读取字节，然后写入液晶
  }
}
...
void Refresh(u8 x,u8 y,u8 width,u8 height)
{
	...
	{
		LCD_set_XY(start_column,height_i);				//从行首开始刷新
		//刷新该行的width列
